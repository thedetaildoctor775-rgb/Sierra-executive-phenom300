...
function selectedAircraftFixed(){
  ...
  return state.fleet[tail]||state.fleet[state.selectedAircraft]||state.fleet.N300SE;
}
...
function reconcileFleetFixed(){
  ...
  // Career totals are company-wide, but location/status belong to the actual aircraft.
  ...
  if(state.activeAssignment && workflowBusy()){
    state.activeAssignment.aircraft=tail;
    state.activeAircraft=tail;
  }

  for(const [t,aircraft] of Object.entries(state.fleet)){
    if(t===tail && workflowBusy()){
      aircraft.status=['departed'].includes(String(state.workflow||'').toLowerCase())?'In Flight':'Assigned';
    }else if(t!==tail && aircraft.status==='Assigned'){
...
function selectAircraftFixed(tail){
  ...
  if(workflowBusy() && state.activeAircraft && state.activeAircraft!==tail){
...
function syncLocationForStep(stepName,tail){
  ...
  if(s==='accepted' || s==='duty' || s==='fuel' || s==='boarded' || s==='ready'){
...
// Replace the legacy hard-coded Phenom-only fleet functions.
...
// Keep aircraft identity + location synchronized whenever workflow advances.
...
if(typeof resetFlight==='function'){
  ...
  resetFlight=function(...args){
    ensureFleetFixed();
    const tail=state.selectedAircraft&&state.fleet[state.selectedAircraft]?state.selectedAircraft:activeTail();
...
// Repair any stale state left by the previous hard-coded N300SE logic.
ensureFleetFixed();
if(!workflowBusy()){
