(function(){
  ...
  function sxWestCoastMarket(silent){ensureFleet110();const base=String(selectedAircraft().location||'KRNO').toUpperCase();state.marketBoard=Array.isArray(state.marketBoard)?state.marketBoard:[];const now=Date.now();const protectedRows=state.marketBoard.filter(x=>x&&(x.protected===true||x.manual===true||x.jobType==='CONTRACT'||x.status==='Accepted'));const used=new Set(protectedRows.map(x=>String(x.flight||'')));let n=nextNumber(used);const destinations=pickDiverseDestinations(base,12);const profiles=shuffle(PROFILES);const generated=[];for(let i=0;i<destinations.length;i++){while(used.has('SXR'+n))n++;const p=profiles[i%profiles.length],dest=destinations[i],distance=approximateDistance(base,dest),pax=rand(p.pax[0],p.pax[1]),bagBase=rand(p.bags[0],p.bags[1]),value=Math.round((p.premium+4200+distance*8.2+pax*275)/50)*50,depDay=i<7?0:(i<11?1:2);generated.push({flight:'SXR'+n,origin:base,dest,client:p.client,pax,bags:sxRealisticBaggage(pax,bagBase),value,expiry:new Date(now+(4.5+i*1.15)*3600000).toISOString(),event:p.event,category:p.category,charterType:p.type,jobType:'MARKET',minClass:distance>1050?'Midsize Jet':distance>650?'Super Light Jet':'Light Jet',distance,departureDate:day(depDay),departureTime:TIMES[i%TIMES.length],status:'Available',westCoastMarket:true});used.add('SXR'+n);n++}state.marketBoard=[...protectedRows,...generated];state.marketRevision='West Coast Diverse Market v2.0';state.marketLocation=base;state.marketSeeded=true;saveState();if(!silent&&typeof renderMarket==='function')renderMarket();if(!silent&&typeof g==='function'&&g('marketStatus'))g('marketStatus').textContent='West Coast charter market active from '+base+' — 12 diverse offers across California, Nevada, Arizona, Oregon, Washington and western business destinations.'}
  const originalSeedMarket=seedMarket;seedMarket=function(silent){originalSeedMarket(silent);sxWestCoastMarket(silent)};window.sxWestCoastMarket=sxWestCoastMarket;window.sxBalancedMarket=sxWestCoastMarket;sxWestCoastMarket(false);setTimeout(()=>sxWestCoastMarket(false),1200);
