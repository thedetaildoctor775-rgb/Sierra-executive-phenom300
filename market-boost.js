(function(){
  ...
  function nm(a,b){
    const A=COORD[a],B=COORD[b]; if(!A||!B)return 0;
    const r=3440.065,toRad=x=>x*Math.PI/180,dlat=toRad(B[0]-A[0]),dlon=toRad(B[1]-A[1]);
  ...
  function install(){
    const frame=document.getElementById('legacyFrame'); if(!frame)return;
    try{
      const w=frame.contentWindow; if(!w||w.__sxNortheastMarketInstalled)return;
      w.__sxNortheastMarketInstalled=true;
      const routeJSON=JSON.stringify(ROUTES),coordJSON=JSON.stringify(COORD),profilesJSON=JSON.stringify(PROFILES),neJSON=JSON.stringify([...NE]);
      w.eval(`(function(){
        if(window.__sxNortheastMarketRuntime)return;
        window.__sxNortheastMarketRuntime=true;
        const SX_NE_ROUTES=${routeJSON};
        const SX_NE_COORD=${coordJSON};
        const SX_NE_PROFILES=${profilesJSON};
        const SX_NE_SET=new Set(${neJSON});
        function sxNm(a,b){const A=SX_NE_COORD[a],B=SX_NE_COORD[b];if(!A||!B)return 0;const r=3440.065,t=x=>x*Math.PI/180,dl=t(B[0]-A[0]),dn=t(B[1]-A[1]),q=Math.sin(dl/2)**2+Math.cos(t(A[0]))*Math.cos(t(B[0]))*Math.sin(dn/2)**2;return Math.round(r*2*Math.atan2(Math.sqrt(q),Math.sqrt(1-q)))}
        function sxDate(days){const d=new Date(Date.now()+days*86400000);return d.toISOString().slice(0,10)}
        function sxNextFlight(used){let n=nextFlight();while(used.has('SXR'+n))n++;return n}
        function sxBoostNortheastMarket(silent){
          ensureFleet110();
          const base=String((selectedAircraft().location||'KVNY')).toUpperCase();
          const routes=SX_NE_ROUTES[base];
          ...
          let n=sxNextFlight(used);
          const target=SX_NE_SET.has(base)?10:8;
          const premium=[];
          for(let i=0;i<SX_NE_PROFILES.length&&premium.length<target;i++){
            while(used.has('SXR'+n))n++;
            const p=SX_NE_PROFILES[i],dest=routes[i%routes.length];
            if(!dest||dest===base)continue;
            const miles=sxNm(base,dest)||Math.round(350+Math.random()*1600);
            ...
            premium.push({
              ...
              departureDate:sxDate(i<5?0:1),departureTime:['07:10','08:20','09:45','11:15','13:05','14:40','16:10','17:35','19:00','20:15'][premium.length%10],
          ...
          state.marketLocation=base;state.marketSeeded=true;
          saveState();
        ...
        const sxOriginalSeedMarket=seedMarket;
        seedMarket=function(silent){sxOriginalSeedMarket(silent);sxBoostNortheastMarket(silent)};
    ...
    }catch(e){console.warn('Sierra Northeast market booster:',e)}
  }
  function attach(){const f=document.getElementById('legacyFrame');if(!f)return;f.addEventListener('load',()=>setTimeout(install,250));setTimeout(install,700)}
